-- 0094: cockpit_data — active_students-Count auf contacts/contact_student umstellen
--
-- Hintergrund: Migration 0093 hat die public.students-View gedroppt (war ein
-- straight SELECT on public.people). Die cockpit_data-RPC aus 0045 referenziert
-- aber noch 'FROM students WHERE active' (Zeile 114 in 0045) → Cockpit-Screen
-- schlägt mit "relation 'students' does not exist" fehl.
--
-- Fix: aktive Schüler aus contacts JOIN contact_student mit
-- archived_at IS NULL zählen — gleiche Logik wie lib/contactQueries.listStudents.
-- Postgres erlaubt kein ALTER FUNCTION BODY, daher wird die komplette Function
-- neu definiert (Body unverändert außer Zeile 114-Äquivalent).
...
BEGIN
  ...
  SELECT json_build_object(
    ...
    -- Phase J Etappe 3c: students-View weg, contacts+contact_student ist die
    -- neue Quelle. archived_at IS NULL = aktiv im neuen Modell.
    'active_students', (
      SELECT COUNT(*)
      FROM contacts c
      JOIN contact_student cs ON cs.contact_id = c.id
