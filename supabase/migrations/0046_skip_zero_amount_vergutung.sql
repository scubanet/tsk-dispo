-- Bug-Fix: Assignment-Insert für Aktivitäten ohne Vergütung schlägt fehl.
--
-- Hintergrund:
--   account_movements hat CHECK (amount_chf <> 0 OR kind = 'übertrag')
--   Wenn ein TL einer "Sonstigen Aktivität" (z.B. Materialtesttag, So-Dive,
--   Tschiggi After-Work, Cleanup) zugewiesen wird, gibt's keine Vergütung →
--   calc_compensation() liefert amount_chf=0 zurück. Der Trigger versucht
--   trotzdem eine vergütung-Bewegung mit 0 CHF zu schreiben → CHECK violation.
--
-- Fix: Trigger schreibt nur eine Bewegung wenn amount_chf <> 0.
-- Für UPDATE wird ebenfalls die 0-Korrektur übersprungen.
...
BEGIN
  IF TG_OP = 'INSERT' THEN
    ...
    -- Aktivitäten ohne Vergütung (z.B. Materialtesttag) → keine Buchung.
  ...
  IF TG_OP = 'UPDATE' THEN
    v_breakdown := calc_compensation(NEW.id);
    v_amount := COALESCE((v_breakdown->>'amount_chf')::numeric, 0);
    ...
    -- Nur korrektur schreiben wenn Delta tatsächlich von 0 abweicht.
    IF v_delta <> 0 THEN
      SELECT start_date INTO v_course_date FROM courses WHERE id = NEW.course_id;
...
COMMENT ON FUNCTION write_movement_for_assignment IS
  'On INSERT writes vergütung (>0 only), on UPDATE writes korrektur for delta, on DELETE reverses.';
